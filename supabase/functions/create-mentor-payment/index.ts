...
serve(async (req) => {
  ...
  try {
    ...
    const { mentor_id, mentor_name, amount, currency, booking_date, start_time, end_time, notes } = await req.json();
    if (!mentor_id || !amount) throw new Error("Missing required fields");
